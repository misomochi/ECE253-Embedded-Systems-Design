...
void QHsmTst_ctor(void) {
	...
	HSM_QHsmTst.emergency_active = false; /*initialize emergency state variable*/
...
QState QHsmTst_elevator(QHsmTst *me) {
	switch (Q_SIG(me)) {
		...
		case TICK_SIG:
			...
			return Q_TRAN(&QHsmTst_stopped);
		case F1_SIG: case F2_SIG: case F3_SIG: case F4_SIG: case F5_SIG: case EMERGENCY_SIG:
...
QState QHsmTst_stopped(QHsmTst *me) {
	switch (Q_SIG(me)) {
    	...
    	case F5_SIG:
    		...
    		return Q_HANDLED();
    	case EMERGENCY_SIG: /*for question 4*/
    		BSP_display("stopped-EMERGENCY\n");
    		for (int i = 0; i < 5; ++i) HSM_QHsmTst.floor_pen[i] = 0; /*Clear the pending calls when emergency is set*/
    		if (HSM_QHsmTst.curr_floor != 0) {                        /* Check if elevator is stopped at 1st floor already*/
    			HSM_QHsmTst.floor_pen[0] = 1;                         /*Mark 1st floor as pending*/
				HSM_QHsmTst.emergency_curr_call_time = simTime;       /*Store the time when it is requested*/
				HSM_QHsmTst.emergency_active = true;
    		}
			HSM_QHsmTst.emergency_calls++;                            /*Increment the number of calls to emergency*/			
    		return Q_HANDLED();
	}
	...
	switch (Q_SIG(me)) {
    	...
    	case TICK_SIG:
    		...
    		if (HSM_QHsmTst.move_time < MOVE_TIME_F-1) HSM_QHsmTst.move_time++;
				...
				if (checkPending() == -1) {
					// STOP_TIME_F = (rand() % 10) + 6; /*for question 3*/
					/*for question 4*/
					if (HSM_QHsmTst.emergency_active) {
						if (HSM_QHsmTst.curr_floor == 0) { /*check if current floor is 1st floor*/
							HSM_QHsmTst.emergency_active = false;
							HSM_QHsmTst.emergency_total_time += (simTime - HSM_QHsmTst.emergency_curr_call_time);
							HSM_QHsmTst.emergency_curr_call_time = -1;
						} else HSM_QHsmTst.floor_pen[0] = 1;
					}
					return Q_TRAN(&QHsmTst_stopped); /*Switch to stopped if the current floor is pending*/ 
				}
    	...
    	case EMERGENCY_SIG: /*for question 4*/
			...
			for (int i = 0; i < 5; ++i) HSM_QHsmTst.floor_pen[i] = 0;                 /*Clear the pending calls when emergency is set*/
    		HSM_QHsmTst.floor_pen[HSM_QHsmTst.curr_floor + HSM_QHsmTst.curr_dir] = 1; /*Mark next floor as pending*/
    		HSM_QHsmTst.emergency_curr_call_time = simTime;                           /*Store the time when emergency is called*/
			HSM_QHsmTst.emergency_calls++;                                            /*Increment the number of calls to emergency*/
			HSM_QHsmTst.emergency_active = true;
    		return Q_HANDLED();
	}
